...
// // Extracting bill details from text using regex
// export const extractBillDetails = (text) => {
//     const amountMatch = text.match(/([\d,]+\.\d{2})/);
//     const consumptionMatch = text.match(/(\d{1,5})\s?(m3|liters|cubic\s?meters|cons)/i);
//     const dateMatch = text.match(/(\d{2})\s([A-Za-z]{3})\s(\d{4})/);
...
// // Function to upload and analyze water bill images
// export const uploadAndAnalyze = async (req, res) => {
//     console.log("Files received:", req.files);
//     console.log("Request Body:", req.body);
...
//             // Perform OCR (Optical Character Recognition) on the uploaded image
//             const { data: { text } } = await Tesseract.recognize(imagePath, "eng");

//             console.log("Extracted Text:", text);

//             // Extract bill details (e.g., amount, consumption, date)
//             const { billAmount, waterConsumption, billDate } = extractBillDetails(text);

//             // Create a new water bill entry
//             const newBill = new WaterBill({
//                 imageUrl: imagePath,
//                 billAmount,
//                 waterConsumption,
//                 billDate,
//                 user: userId // Associate the bill with the logged-in user
//             });

//             // Save the water bill to the database
//             await newBill.save();
//             bills.push(newBill);
//         }
